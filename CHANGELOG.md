...
### Added
- \mod_vimipad\api\submissions: a public facade answering who contributed to a
  submitted snapshot. A plugin copying a group map elsewhere needs this so its own
  privacy provider can find and anonymise the contribution, and it should not
  have to query the operation log directly.

## 0.9.19 - 2026-08-12

### Fixed
- phpmd.xml excluded five CamelCase rules but not CamelCaseNamespace, so every
  namespaced class and every deliberately global file still produced a finding.
  The same reason applies: Moodle prescribes frankenstyle namespaces with
  snake_case sub-namespaces, and many files are global by design.

### Changed
- The AMD target can refresh the browserslist database with
  BROWSERSLIST_UPDATE=1. Off by default: it writes to the lockfile and reaches
  the network, and a stale caniuse-lite only warns, never builds wrongly.

## 0.9.18 - 2026-08-12

### Fixed
- api\value read a node's shape from a top-level "shape" key, falling back to
  "type". Neither is where a shape lives: "type" is the profile's semantic node
  type (for example "concept"), and the visual shape sits in metadatajson using
  the vocabulary node_style defines (roundrect, rect, ellipse). The policy could
  therefore reject a perfectly ordinary node when a consumer restricted shapes,
  and never checked the real shape at all when it did not. Shapes are now read
  from metadatajson and checked against the profile first; a consumer may narrow
  the profile's set but never widen it.
- Node metadata is validated (through node_style) and bounded, and element
  collections must be real lists rather than any array.
- package.json and package-lock.json were still on 0.9.14.
- README claimed the core plugin and its subplugins share one release; they are
  versioned independently on purpose.

### Added
- docs/backlog.md: a persistent list of open items across all four plugins.
  Previously these lived only in session notes and were lost between rounds.

## 0.9.17 - 2026-08-12

### Changed
- Added phpmd.xml, a curated PHPMD ruleset. PHPMD's stock rules assume a
  PSR-style codebase and several of them contradict the Moodle coding standard
  that phpcs enforces, so their output buried the real findings. Each exclusion
  carries its reason in the file; size and complexity rules are deliberately kept.
- api\value::validate() was split into per-section checks. It had grown to
  cyclomatic complexity 44 and an NPath complexity of 4.5 billion, which is far
  too much for the one place every consumer plugin relies on for validation.
- mod_form::definition() split into availability and constraint sections.
- Removed unused globals, hoisted three assignments out of if conditions, and
  initialised the by-reference out-parameters of the workspace validators before
  the call, so both readers and static analysis can see where they come from.
- Remaining PHPMD findings (59, all size or complexity in older service and
  output code) are tracked with a priority order in docs/phpmd-backlog.md rather
